		...
		{					
			if (isPlayer _x) then 
			{
				...
				{ // Players 
					...
					if(_OriginalSide == civilian) then {lbSetColor[KEGs_cLBTargets, _i, [1,0.8,0.8,1]]};
					if(_OriginalSide == independent) then {lbSetColor[KEGs_cLBTargets, _i, [0.8,1,0.8,1]]};
					if(_OriginalSide == east) then {lbSetColor[KEGs_cLBTargets, _i, [1,1,1,1]]};								
				/*}
				else
				{
