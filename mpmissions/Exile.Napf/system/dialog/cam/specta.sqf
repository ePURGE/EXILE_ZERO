...
5 cutText ["Initializing Spectator Script...","BLACK IN", 2];
...
spectate_events = compile preprocessFileLineNumbers format ["%1system\dialog\cam\specta_events.sqf",MCC_path];
RefreshPlayerList = compile preprocessFileLineNumbers format ["%1system\dialog\cam\RefreshPlayerList.sqf",MCC_path];
FreeLookMovementHandler = compile preprocessFileLineNumbers format ["%1system\dialog\cam\FreeLookMovementHandler.sqf",MCC_path];
CameraMenuHandler = compile preprocessFileLineNumbers format ["%1system\dialog\cam\CameraMenuHandler.sqf",MCC_path];
PlayerMenuHandler = compile preprocessFileLineNumbers format ["%1system\dialog\cam\PlayerMenuHandler.sqf",MCC_path];
KEGsShowCombatMode = compile preprocessFileLineNumbers format ["%1system\dialog\cam\specta_combatmode.sqf",MCC_path];
KEGsShowUnitLocator = compile preprocessFileLineNumbers format ["%1system\dialog\cam\specta_locator.sqf",MCC_path];
...
VM_CheckOriginalSide =  
{
	...
	_OriginalSide = _unit getVariable ["KEG_OriginalSide", west];
...
//[] spawn {sleep(3);if(dialog) then {5 cutText["\n\n\n\n\nPress F1 for help","PLAIN DOWN", 0.75]}};
[] spawn {sleep(3);if(dialog) then {5 cutText["\n\n\n\n\nPress ESC for exit","PLAIN DOWN", 0.75]}};
	...
	CheckNewUnits = 
		{
			...
			if(count _newUnits > 0) then 
			{
				...
				// Create markers
				{
					...
					if(_OriginalSide == civilian) then {_m setMarkerColorLocal "ColorRed";};
					if(_OriginalSide == independent) then {_m setMarkerColorLocal "ColorGreen";};
					if(_OriginalSide == east) then {_m setMarkerColorLocal "ColorWhite";};			
					...
					} else { _markers = []; diag_log format ["spectator script: '_markers' error for unit [%1]", _x]; };
...
while{ dialog } do 
{
		...
		if(time - KEGs_lastAutoUpdateLB > 60 && (RefreshListReady) && (CheckNewUnitsReady)) then 
		{
			KEGs_lastAutoUpdateLB = time;
			KEGsNeedUpdateLB = true;
		};	
		...
		if((time - lastCheckNewUnits > 60)  && (CheckNewUnitsReady) ) then 
		{
			lastCheckNewUnits = time;
			RefreshListReady = false;
			[] spawn CheckNewUnits;
