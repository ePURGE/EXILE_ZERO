...
//_cnt = count nearestLocations [getPosATL player, ["NameVillage","NameCity","NameCityCapital"], 100];
/*edit*/
try 
{
	/*edit*/
	...
	*/
	if ([_position] call exsys_fnc_checkBuild) then
	{
		throw 3;
	};
	/*edit*/
	...
	{
		{
			...
			if (((AGLtoASL _position) distance (getPosASL _x)) < _radius) then
			{
				...
				if (_playerUID in _buildRights) then
				{
					...
					_numberOfConstructionsAllowed = ((_territoryLevelConfig select 2) + 1);
