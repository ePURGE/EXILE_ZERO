...
ExileClientPlayerIsHungry = (ExileClientPlayerAttributes select 2) < 6;
ExileClientPlayerIsThirsty = (ExileClientPlayerAttributes select 3) < 6;
...
if (ExileClientPlayerIsInfantry) then 
{
	...
	{
		_hungerFactor = 1 + ExileClientPlayerVelocity / 128 * _timeElapsed; 
		_thirstFactor = 1 + ExileClientPlayerVelocity / 96 * _timeElapsed; 
	};
}
else 
{
...
/*edit*/
if (diag_tickTime - ExileClientPostProcessingBackgroundBlur_wert_timer > 5) then
{
		
	if((ExileClientPostProcessingBackgroundBlur_wert isEqualTo 0) && {!ExileClientPostProcessingBackgroundBlur_wert_on && {(_hunger <= 6 || _thirst <= 6)}})then
	{
		ExileClientPostProcessingBackgroundBlur_wert_on = true;
		ExileClientPostProcessingBackgroundBlur_wert = 1.0;
		...
		ExileClientPostProcessingBackgroundBlur ppEffectAdjust [ExileClientPostProcessingBackgroundBlur_wert];
		ExileClientPostProcessingBackgroundBlur ppEffectCommit 5;
	} 
	else 
	{
		if((ExileClientPostProcessingBackgroundBlur_wert isEqualTo 1.0) && {ExileClientPostProcessingBackgroundBlur_wert_on})then
		{
		ExileClientPostProcessingBackgroundBlur_wert_on = false;
		ExileClientPostProcessingBackgroundBlur ppEffectAdjust [ExileClientPostProcessingBackgroundBlur_wert];
		ExileClientPostProcessingBackgroundBlur ppEffectCommit 0;
		ExileClientPostProcessingBackgroundBlur ppEffectEnable true;
		ExileClientPostProcessingBackgroundBlur ppEffectAdjust [0];
		ExileClientPostProcessingBackgroundBlur ppEffectCommit 5;
		ExileClientPostProcessingBackgroundBlur_wert = 0;
		};
	};
		
	ExileClientPostProcessingBackgroundBlur_wert_timer = diag_tickTime;
};
/*edit*/
ExileLastStatUpdate = diag_tickTime;
true