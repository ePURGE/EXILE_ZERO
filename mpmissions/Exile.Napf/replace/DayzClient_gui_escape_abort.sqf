...
private["_display","_controlAbort","_controlSuicide","_controlTitle","_controlFiledShit","_abortTime","_respawnTime"];
...
if !(alive player) then
{
	...
	/*edit*/
		while {!isNull _display} do
		{
			...
			_controlSuicide ctrlSetText format ["For Respawn wait ( %1 sec )", ceil(_respawnTime - diag_tickTime)];
			_controlSuicide ctrlCommit 0;
...
else 
{
	...
	else 
	{
		...
		{
			_controlAbort ctrlEnable false;
			_controlAbort ctrlSetText format ["Please wait ( %1 sec )", ceil(_abortTime - diag_tickTime)];
			_controlAbort ctrlCommit 0;
