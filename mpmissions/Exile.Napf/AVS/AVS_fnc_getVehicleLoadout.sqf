...
{
	diag_log format ["AVS Error: Calling AVS_fnc_saveAmmo - _vehicle is Null!"];
};
...
//diag_log format ["AVS INFO: Calling AVS_fnc_getVehicleLoadout - parameters: vehicle=%2 load=%1",(magazinesAllTurrets _vehicle),(typeOf _vehicle)];
...
if (isNil "_currentLoadout")then {_currentLoadout = [];};
